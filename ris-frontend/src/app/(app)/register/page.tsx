...
export default function RegistrationWorkflowPage() {
  ...
  const fetchConfiguration = async () => {
    try {
      const [wardsRes, disciplinesRes, examsRes] = await Promise.all([
  ...
  const completeRegistration = async () => {
    ...
    try {
      const payload = {
        ...
        discipline: parseInt(registrationData.discipline),
        doctor: registrationData.doctor,
        clinical_notes: registrationData.clinical_notes,
        urgency: registrationData.urgency,
        examinations: selectedExams.map(ex => ({
          exam: ex.exam,
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select value={registrationData.discipline} onValueChange={(value) => setRegistrationData({...registrationData, discipline: value})}>
                    <SelectTrigger>
                      <SelectValue placeholder="Select discipline" />
                    </SelectTrigger>
                    <SelectContent>
                      {disciplines.map((disc) => (
                        <SelectItem key={disc.id} value={disc.id.toString()}>{disc.nama}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div className="space-y-2">
                  <Label>Urgency</Label>
                  <Select value={registrationData.urgency} onValueChange={(value) => setRegistrationData({...registrationData, urgency: value})}>
                    <SelectTrigger>
